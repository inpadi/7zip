...
// PackedSize returns the compressed size charged to this file by 7-Zip's
// listing convention. In a solid folder only the first file carries the
// folder's packed size.
func (f *File) PackedSize() (uint64, bool) {
	if f.isEmptyStream || f.isEmptyFile {
		return 0, false
	}
	if f.zip == nil || f.zip.si == nil || f.zip.si.packInfo == nil || f.zip.si.unpackInfo == nil ||
		f.offset != 0 || f.folder < 0 || f.folder >= f.zip.si.Folders() {
		return 0, false
	}
	packedOffset := 0
	for i := 0; i < f.folder; i++ {
		packedOffset += len(f.zip.si.unpackInfo.folder[i].packed)
	}
	packedStreams := len(f.zip.si.unpackInfo.folder[f.folder].packed)
	if packedOffset+packedStreams > len(f.zip.si.packInfo.size) {
		return 0, false
	}
	var size uint64
	for _, packed := range f.zip.si.packInfo.size[packedOffset : packedOffset+packedStreams] {
		size += packed
	}
	return size, true
}
...
func (f *File) Open() (io.ReadCloser, error) {
	...
	if rc == nil {
		...
		rc, _, encrypted, err = f.zip.folderReader(f.zip.si, f.folder, false)
		if err != nil {
...
func (z *Reader) folderReader(si *streamsInfo, f int, checksum bool) (*folderReadCloser, uint32, bool, error) {
	// Create a SectionReader covering all of the streams data
	return si.folderReader(io.NewSectionReader(z.r, z.start, z.end-z.start), f, z.p, checksum)
}
...
func (z *Reader) init(r io.ReaderAt, size int64) (err error) {
	...
	if streamsInfo != nil {
		...
		fr, crc, encrypted, err = z.folderReader(streamsInfo, 0, true)
		if err != nil {
			return &ReadError{
				Encrypted: encrypted,
				Err:       err,
