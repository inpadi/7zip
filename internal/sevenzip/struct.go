...
func (rc *folderReadCloser) Seek(offset int64, whence int) (int64, error) {
	...
	case io.SeekStart:
		newo = offset
	case io.SeekCurrent:
		newo = int64(rc.count) + offset //nolint:gosec
	case io.SeekEnd:
		newo = rc.Size() + offset
	default:
	...
	if _, err := io.CopyN(io.Discard, rc, newo-int64(rc.count)); err != nil { //nolint:gosec
...
func newFolderReadCloser(rc io.ReadCloser, size int64, hasEncryption, checksum bool) *folderReadCloser {
	nrc := &folderReadCloser{rc: rc, size: size, hasEncryption: hasEncryption}
	if checksum {
		nrc.h = crc32.NewIEEE()
	}
	return nrc
