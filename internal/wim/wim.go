...
const supportedHdrFlags = hdrFlagRpFix | hdrFlagReadOnly | hdrFlagCompressed |
	hdrFlagCompressXpress | hdrFlagCompressLzx
...
func NewReader(f io.ReaderAt) (*Reader, error) {
	...
	methodFlags := r.hdr.Flags & (hdrFlagCompressXpress | hdrFlagCompressLzx)
	if r.hdr.Flags&hdrFlagCompressed != 0 {
		if methodFlags != hdrFlagCompressXpress && methodFlags != hdrFlagCompressLzx {
			return nil, fmt.Errorf("unsupported WIM compression flags %x", methodFlags)
		}
	} else if methodFlags != 0 {
		return nil, fmt.Errorf("WIM compression method %x set on an uncompressed image", methodFlags)
	}

	if r.hdr.Flags&hdrFlagCompressed != 0 && r.hdr.CompressionSize != 0x8000 {
...
func (r *Reader) resourceReaderWithOffset(hdr *resourceDescriptor, offset int64) (io.ReadCloser, error) {
	...
	if hdr.Flags()&resFlagCompressed == 0 {
		...
		method := compressionLZX
		if r.hdr.Flags&hdrFlagCompressXpress != 0 {
			method = compressionXPRESS
		}
		cr, err := newCompressedReader(section, hdr.OriginalSize, offset, method)
		if err != nil {
			return nil, err
		}
		sr = cr
...
func (*Reader) readSecurityDescriptors(rsrc io.Reader) (sds [][]byte, n int64, err error) {
	...
	// Some older WIM producers use zero instead of the header size for an
	// empty security table. The directory data still starts after the header.
	if secBlock.TotalLength == 0 && secBlock.NumEntries == 0 {
		return sds, n, nil
	}
	...
	if n > secsize {
		return sds, n, &ParseError{
			...
			Err:  fmt.Errorf("security descriptor table size %d is smaller than its %d-byte header and entries", secsize, n),
		}
	}
