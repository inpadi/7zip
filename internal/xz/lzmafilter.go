...
func (f lzmaFilter) reader(r io.Reader, c *ReaderConfig) (fr io.Reader,
	...
	if dc > config.DictCap {
		return nil, fmt.Errorf("xz: LZMA2 dictionary capacity %d exceeds the %d-byte limit", dc, config.DictCap)
	}
	if lzmadec.Available() {
		property := []byte{lzma.EncodeDictCap(f.dictCap)}
		fr, err = lzmadec.NewLZMA2Unknown(
			io.NopCloser(r), property, uint64(config.DictCap),
		)
		if err != nil {
			return nil, err
		}
