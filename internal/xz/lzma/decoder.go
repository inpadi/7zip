...
func (d *decoder) readOp() (op operation, err error) {
	...
	if b == 0 {
		...
		op = matchOperation(match{n: int(n) + minMatchLen,
	...
	if b == 0 {
		...
		if b == 0 {
			...
			op = matchOperation(match{n: 1, distance: int64(dist) + minDistance})
			return op, nil
		}
	} else {
	...
	op = matchOperation(match{n: int(n) + minMatchLen, distance: int64(dist) + minDistance})
	return op, nil
}
