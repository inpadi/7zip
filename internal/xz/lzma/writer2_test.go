...
import (
	...
	"github.com/inpadi/7zip/internal/xz/internal/randtxt"
)
...
func TestReader2DoesNotDiscardUnreadChunkBytes(t *testing.T) {
	payload := bytes.Repeat([]byte("compressed chunk boundary "), 4096)
	var encoded bytes.Buffer
	w, err := (Writer2Config{DictCap: 4096}).NewWriter2(&encoded)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(stream) < 7 || stream[0] < 0x80 {
		t.Fatalf("first chunk is not compressed: %x", stream[:min(len(stream), 7)])
	}
	headerSize := 5
	if stream[0] >= 0xc0 {
		headerSize++
	}
	compressedSize := int(stream[3])<<8 | int(stream[4])
	compressedSize++
	bodyEnd := headerSize + compressedSize
	if bodyEnd >= len(stream) {
		t.Fatalf("compressed chunk ends at %d in %d-byte stream", bodyEnd, len(stream))
	}
	stream = append(stream, 0)
	copy(stream[bodyEnd+1:], stream[bodyEnd:])
	stream[bodyEnd] = 0x7f
	compressedSize++
	encodedSize := compressedSize - 1
	stream[3], stream[4] = byte(encodedSize>>8), byte(encodedSize)

	r, err := (Reader2Config{DictCap: 4096}).NewReader2(bytes.NewReader(stream))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := io.ReadAll(r); err == nil {
		t.Fatal("reader accepted an unread byte hidden inside the compressed chunk")
	}
}
